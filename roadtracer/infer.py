...
def roadtracer_infer(m, session, img, starting_locs):
    ...
    rect = geom.Rectangle(geom.Point(-200, -200), geom.Point(200, 200))

    tile_data = {
        'region': 'does_this_even_matter',
        'rect' : rect,
        'search_rect': rect,
        # 'search_rect': rect.add_tol(-WINDOW_SIZE//2),
        'cache': tileloader.TileCache(limit=16, mode=TILE_MODE),
        'starting_locations': [],
        'gc': graph.GraphContainer(global_graph),
    }
    paths = [model_utils.Path(tile_data['gc'], tile_data, g=global_graph)]

    for start in starting_locs:
        global_graph.add_vertex(start)
        
    for vertex in global_graph.vertices:
        paths[0].prepend_search_vertex(vertex)

    path_lengths = {path_idx: 0 for path_idx in range(len(paths))}

    img_padded = np.zeros((WINDOW_SIZE + 400, WINDOW_SIZE + 400, 3))
    img_padded[(WINDOW_SIZE//2) : (WINDOW_SIZE//2 + 400), (WINDOW_SIZE//2) : (WINDOW_SIZE//2 + 400), :] = img
    ...
    for len_it in range(99999999):
        ...
        for i in range(len(path_indices)):
            ...
            path = paths[path_idx]
            tile_point = np.zeros((WINDOW_SIZE, WINDOW_SIZE, 1), dtype='float32')
            ...
            origin = extension_vertices[i].point.sub(geom.Point(WINDOW_SIZE/2, WINDOW_SIZE/2))
            tile_origin = origin.sub(big_origin)

            rect = origin.bounds().extend(origin.add(geom.Point(WINDOW_SIZE, WINDOW_SIZE)))

            tile_path = np.zeros((WINDOW_SIZE, WINDOW_SIZE, 1), dtype='float32')
            ...
            for edge_id in path.edge_rtree.intersection((rect.start.x, rect.start.y, rect.end.x, rect.end.y)):
                ...
                for p in geom.draw_line(start.sub(origin), end.sub(origin), geom.Point(WINDOW_SIZE, WINDOW_SIZE)):
                    tile_path[p.x, p.y, 0] = 1.0

            path_input = np.concatenate([img_padded[(WINDOW_SIZE//2 + tile_origin.x):(tile_origin.x + 3 * WINDOW_SIZE//2), (WINDOW_SIZE//2 + tile_origin.y):(tile_origin.y + 3 * WINDOW_SIZE//2), :], tile_path, tile_point], axis=2)
        ...
        batch_angle_outputs, batch_stop_outputs, batch_detect_outputs = session.run([m.angle_outputs, m.action_outputs, m.detect_outputs], feed_dict=feed_dict)
